...
setMethod("as.speeches", "partition", function(
  ...
  if (n_dates > 1L){
    ...
    while(s_attribute_date %in% names(slot(.Object, "s_attributes")))
      slot(.Object, "s_attributes")[[s_attribute_date]] <- NULL
    ...
    if (!s_attribute_date %in% names(slot(.Object, "s_attributes"))){
      date_values <- s_attributes(.Object, s_attribute = s_attribute_date)
      slot(.Object, "s_attributes")[[s_attribute_date]] <- date_values
    }
  ...
  partition_names <- sapply(
    ...
    function(x){
      paste(
        slot(x, "s_attributes")[[s_attribute_name]],
        slot(x, "s_attributes")[[s_attribute_date]],
        gsub("^.*_(\\d+)$", "\\1", slot(x, "name")),
        sep = "_", collapse = "_"
      )
    }
  )
  ...
  speaker_list_ordered <- lapply(
    order(sapply(speaker_list, function(x) slot(x, "cpos")[1,1])),
    function(i) speaker_list[[i]]
  )
  corpus <- if (is.character(.Object)) .Object else slot(.Object, "corpus")
  properties <- corpus_properties(
    corpus = slot(.Object, "corpus"),
    registry = slot(.Object, "registry_dir")
  )
  if ("type" %in% properties){
    type <- corpus_property(slot(.Object, "corpus"), slot(.Object, "registry_dir"), "type")
    if (type == "plpr"){
...
setMethod("as.speeches", "subcorpus", function(
  ...
  obj_names <- sapply(
    ...
    function(x){
      ...
      paste(
        name, date, gsub("^.*_(\\d+)$", "\\1", slot(x, "name")),
        sep = "_", collapse = "_"
      )
    }
  )
...
setMethod("as.speeches", "corpus", function( 
  ...
  regions <- s_attr_regions(
    corpus = slot(.Object, "corpus"), s_attr = s_attribute_name,
    registry = slot(.Object, "registry_dir"), data_dir = slot(.Object, "data_dir")
  )
  ...
  new_class <- if (length(slot(.Object, "type")) == 0L) "subcorpus" else paste(slot(.Object, "type"), "subcorpus", sep = "_")
  ...
  .iter_fn <- function(i){
    ...
    if (nrow(mx) == 1L){
      ...
      slot(y, "strucs") = chunks_strucs[[i]]
      slot(y, "cpos") = mx
      slot(y, "s_attributes") = setNames(
        list(chunks_dates[[i]], names(chunks_cpos)[i]),
        nm = c(s_attribute_date, s_attribute_name)
      )
      slot(y, "xml") = "flat"
      slot(y, "s_attribute_strucs") = s_attribute_name
      slot(y, "name") = sprintf("%s_%s_%d", names(chunks_cpos)[[i]], chunks_dates[[i]], 1L)
      slot(y, "size") = mx[,2] - mx[,1] + 1L
    ...
    lapply(
      ...
      function(j){
        ...
        slot(y, "strucs") = li_strucs[[j]]
        slot(y, "cpos") = matrix(data = li_cpos[[j]], byrow = FALSE, ncol = 2L)
        slot(y, "s_attributes") = setNames(
        ...
        slot(y, "xml") = "flat"
        slot(y, "s_attribute_strucs") = s_attribute_name
        slot(y, "name") = sprintf("%s_%s_%d", names(chunks_cpos)[i], vec_dates[[j]], speech_no[[j]])
        slot(y, "size") = sum(slot(y, "cpos")[,2] - slot(y, "cpos")[,1] + 1L)
  ...
  slot(retval, "xml") <- if (sibling) "flat" else "nested"
  slot(retval, "objects") <- unlist(y, recursive = FALSE)
  names(slot(retval, "objects")) <- sapply(slot(retval, "objects"), name)
