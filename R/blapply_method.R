    ...
    if (requireNamespace(backend, quietly=TRUE)){
      if (backend == "parallel"){
        if (progress == FALSE){
          ...
          startTime <- Sys.time()
          for (core in c(1:noCores)){
            assign(
              threadNames[core],
              mcparallel(
                lapply(c(1:length(xChunks[[core]])), function(i) fWrapped(i=i, core=core, x=xChunks[[core]][[i]], ...))
                ),
              envir=environment())
          }
...
#' @exportClass simple_triplet_matrix
if (require("bigmemory", quietly = TRUE) && require("bigmemory", quietly = TRUE)){
  setClass("simple_triplet_matrix")
  setClass("big.matrix")
  setClassUnion(
    name = "virtualMatrixClass",
    members = c("matrix", "simple_triplet_matrix", "big.matrix")
    )
} else if (require("slam", quietly = TRUE)){
  setClassUnion(
    name = "virtualMatrixClass",
    members = c("matrix", "simple_triplet_matrix")
  )
} else {
  setClassUnion(
    name = "virtualMatrixClass",
