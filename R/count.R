  ...
  if (is.null(query)){
    if (length(pAttribute) == 1){
      ...
      if (decode == FALSE){
        ...
        TF[, "token" := as.nativeEnc(CQI$id2str(.Object, pAttribute, 0:(nrow(TF) - 1)), from = registry_get_encoding(.Object)), with = TRUE]
    ...
    if (cqp == FALSE){
      ...
      if (breakdown == FALSE){
        count <- sapply(
          query,
          function(query){
            cpos_matrix <- cpos(.Object, query, cqp = cqp, pAttribute = pAttribute, encoding = registry_get_encoding(.Object))
