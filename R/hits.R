...
#' y <- corpus("REUTERS") %>%
#'   hits(query = "oil", s_attribute = "places", freq = TRUE)
#'
...
setMethod(
  ...
  function(
    ...
    if (!missing(s_attribute)) {
      if (length(decode) == 1L) {
        ...
        if (length(decode) != length(s_attribute))
          stop(
            "Length of vectors or argument `decode` and `s_attribute` differ"
          )
      }
    }
    ...
    if (fill) {
      # Using s_attributes() requires decoding s-attributes and is potentially
      # not the fastest solution, but it ensures that only combinations that do
      # occur are reported
...
setMethod(
  ...
  function(
    ...
    p_attribute = "word", size = FALSE, freq = FALSE, decode = TRUE,
    by = c("query", "match"),
    mc = FALSE, verbose = TRUE, progress = FALSE, ...
  ) {
  hits(
...
setMethod(
  ...
  function(
    .Object, query, cqp = FALSE, check = TRUE, s_attribute,
    p_attribute = "word", size = FALSE, freq = FALSE, fill = FALSE,
    decode = TRUE, by = c("query", "match"),
...
setMethod("hits", "partition_bundle", function(
  .Object, query, cqp = FALSE, check = TRUE,
  p_attribute = getOption("polmineR.p_attribute"), s_attribute, size = TRUE,
  freq = FALSE, by = c("query", "match"),
...
setMethod(
  "hits", "context",
  function(.Object, s_attribute = NULL, verbose = TRUE, ...) {
  ...
  DT <- ctxt_min[, .get_cpos_left_right(.SD), by = "match_id"]
