...
#' @param cposAsNames logical, whether to return cpos as names of the tokens
...
#' @rdname getTokenStream-method
setMethod("getTokenStream", "matrix", function(.Object, corpus, pAttribute, encoding, collapse=NULL, cposAsNames=FALSE){
  ...
  if (cposAsNames == TRUE) names(tokens) <- cpos
  if (!is.null(collapse)) tokens <- paste(tokens, collapse=collapse)  
  return(tokens)
})

#' @rdname getTokenStream-method
setMethod("getTokenStream", "partition", function(.Object, pAttribute, collapse=NULL, cposAsNames=FALSE){
  getTokenStream(
    .Object=.Object@cpos, corpus=.Object@corpus, pAttribute=pAttribute,
    encoding=.Object@encoding, collapse=collapse, cposAsNames=cposAsNames
    )
})
