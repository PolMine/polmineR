...
setMethod("dotplot", "textstat", function(.Object, col, n = 20L, ...){
  ...
  dotchart(x = slot(.Object, "stat")[[col]][n:1L], labels = slot(.Object, "stat")[[slot(.Object, "p_attribute")[1]]][n:1L], ...)
})

#' @rdname dotplot-method
setMethod("dotplot", "features", function(.Object, col = NULL, n = 20L, ...){
  if (is.null(col)) col <- slot(.Object, "method")[1]
...
setMethod("dotplot", "features_ngrams", function(.Object, col = NULL, n = 20L, ...){
  if (is.null(col)) col <- slot(.Object, "method")[1]
  dt_subset <- slot(.Object, "stat")[n:1L][,grep("^\\d+_.*$", colnames(.Object)), with = FALSE][,n := 1L:.N]
  ngram_vector <- dt_subset[,paste(unlist(.SD[1,]), collapse = " "), by = c("n")][["V1"]]
  dotchart(x = slot(.Object, "stat")[[col]][n:1L], labels = ngram_vector, ...)
})
