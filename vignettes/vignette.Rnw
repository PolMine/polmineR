...
\begin{document}
\title{An introduction to the \textit{polmineR-package} (v0.4.41)}
...
corpus(session) <- "PLPRBTTXT"
leftContext(session) <- 15
rightContext(session) <- 15
multicore(session) <- FALSE
@
