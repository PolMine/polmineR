...
\begin{document}
\title{An introduction to the \textit{driller} (v0.4.19)}
...
bt <- partition(list(text_type="speech"), "PLPRBTTXT")
cdu <- partition(
...
m[c("Integration", "Zuwanderung", "Ausländer", "Migration"),]
@
...
foo <- capture.output(oneQuery <- distribution(
  coalition,
  '"Gerechtigkeit"',
  "text_party"
  ))
# # multiple queries / one dimension
foo <- capture.output(twoQueries <- distribution(
  coalition,
  c('"[eE]uro.*"', '"Br.ssel"'),
  "text_party"
  ))
...
round((twoDim@total$rel*100000),2)[,c(1,2,3,4,6)]
@
