...
if ("CORPUS_REGISTRY" %in% names(Sys.getenv())){
  if (require(rcqp, quietly = T) && require(polmineR.sampleCorpus, quietly = T)) execute <- TRUE
}
```
...
(a) "WARNING: this package has a configure script / It probably needs manual configuration".
(b) The environment variable CORPUS_REGISTRY is not defined.
(c) package 'rcqp' is not installed for 'arch = x64'.
